        ...
        <div className="grid md:grid-cols-2 gap-10 p-10">
          ...
          <div className="flex flex-col justify-between space-y-8">
            ...
            <div>
              ...
              {product.stock !== null && <p className={`mt-2 text-lg font-semibold ${product.stock > 0 ? "text-green-600" : "text-red-600"}`}>{product.stock > 0 ? `${product.stock} items in stock` : "Out of stock"}</p>}
            </div>
            ...
            <div>
              ...
              <div className="inline-flex border border-gray-300 rounded-lg overflow-hidden">
                <button onClick={() => handleQuantityChange(quantity - 1)} className="px-5 py-3 bg-gray-100 hover:bg-gray-200" disabled={quantity <= 1}><FiMinus className="w-6 h-6 text-gray-600" /></button>
                <span className="px-8 py-3 bg-white text-xl font-semibold">{quantity}</span>
                <button onClick={() => handleQuantityChange(quantity + 1)} className="px-5 py-3 bg-gray-100 hover:bg-gray-200" disabled={product.stock !== null && quantity >= product.stock}><FiPlus className="w-6 h-6 text-gray-600" /></button>
              </div>
            </div>

            <div className="flex flex-col sm:flex-row gap-5 mt-8">
              <button onClick={handleBuyNow} disabled={product.stock === 0} className="flex-1 bg-indigo-600 text-white font-semibold py-4 rounded-xl shadow-lg hover:bg-indigo-700">Buy Now</button>
              <button onClick={handleAddToCart} disabled={product.stock === 0} className="flex-1 flex items-center justify-center gap-3 bg-gray-900 text-white font-semibold py-4 rounded-xl shadow-lg hover:bg-gray-800">
          ...
          <div className="border-t border-gray-200 px-10 py-8 bg-gray-50 rounded-b-3xl mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {Object.entries(specs).map(([key, value]) => (
                <div key={key} className="bg-white p-6 rounded-2xl shadow-md hover:shadow-lg">
